# This program handles expense registration with field validation and calendar-based date selection.
...
def add_expense_window(category_handler):
    ...
    window = sg.Window("Add Expense", layout, size=(400, 200))
    while True:
        ...
        if event == "Save":
            ...
            sg.popup(f'Expense "{title}" saved successfully.')
            break
    window.close()

def delete_expense_window(title_list):
    layout = [
        [sg.Text("Select the expense to delete")],
        [sg.Combo(values=title_list, key="Title", readonly=True, size=(30, 1))],
        [sg.Button("Delete"), sg.Button("Cancel")]
    ]
    window = sg.Window("Delete exepnse", layout, size=(400, 200))
    while True:
        event, values = window.read()
        if event in (sg.WIN_CLOSED, "Cancel"):
            break
        if event == "Delete":
            title = values["Title"]
            if not title:
                sg.popup_error("Please select an expense to delete.")
                continue
            ...
            if confirm == "Yes":
                try:
                    ...
                    sg.popup(f'Expense "{title}" deleted successfully.')
                except ValueError as error:
                    sg.popup_error(str(error))
            break
    window.close()

def main_expense_window(data, categories, transaction_handler, category_handler):
    headings = ["Category","Date", "Title", "Amount", "Type"]
    row_colors = []
    table_data = []
    calculator = Calculations.get_calculator(data)
    expense_data = {key: entry for key, entry in data.items() if entry.get("type") == "Expense"}
    build_table(expense_data, categories, row_colors, table_data)
    layout = [
        [sg.Button("Add Expense"), sg.Button("Delete Expense"), sg.Button("Cancel")],
    ...
    window = sg.Window("Expense Manager", layout, size=(500, 400))
    while True:
        event, values = window.read()
        if event in (sg.WIN_CLOSED, "Cancel"):
            break
        if event == "Add Expense":
            add_expense_window(category_handler)
        elif event == "Delete Expense":
            titles = [entry.get("title", "") for entry in data.values() if entry.get("type") == "Expense"]
            delete_expense_window(titles)
